...
<p align="center">
  Agent Harbor は qwibitai と貢献者による NanoClaw のフォークです。安全なコンテナ分離はそのままに、ローカルの llama.cpp バックエンドや Groq のようなホスト型 API も使えるようにして、モデルを置きたい場所で同じアシスタントを動かせるようにしています。
</p>
...
Agent Harborは同じコア機能を提供しますが、理解できる規模のコードベースで実現しています：1つのプロセスと少数のファイル。NanoClawのコンテナ分離を維持しながら、ローカルの llama.cpp サーバーや Groq のような外部 API を、コンテナへ秘密情報を渡さずに使えるようにしました。
...
**理解できる規模。** 1つのプロセス、少数のソースファイル、マイクロサービスなし。Agent Harborのコードベース全体を理解したい場合は、Claude Codeに説明を求めるだけです。
...
**個人ユーザー向け。** Agent Harborはモノリシックなフレームワークではなく、各ユーザーのニーズに正確にフィットするソフトウェアです。肥大化するのではなく、オーダーメイドになるよう設計されています。自分のフォークを作成し、Claude Codeにニーズに合わせて変更させます。
...
**最高のハーネス、柔軟なモデルバックエンド。** Agent HarborはClaude Agent SDK上で動作します。つまり、Claude Codeを直接実行しているということです。Claude Codeは高い能力を持ち、そのコーディングと問題解決能力によってAgent Harborを変更・拡張しつつ、必要に応じてローカルの llama.cpp やホスト型 API に切り替えられます。
...
- **モデルバックエンドの柔軟性** - ローカルの llama.cpp や Groq のようなホスト型 API を、秘密情報をコンテナに渡さずに使えます。
...
Agent Harborは設定ファイルを使いません。変更するには、Claude Codeに伝えるだけです：
...
Telegram対応を追加したい場合、コアコードベースにTelegramを追加するPRを作成しないでください。代わりに、Agent Harborをフォークし、ブランチでコード変更を行い、PRを開いてください。あなたのPRから`skill/telegram`ブランチを作成し、他のユーザーが自分のフォークにマージできるようにします。
...
設定の肥大化を避けたいからです。すべてのユーザーがAgent Harborをカスタマイズし、汎用的なシステムを設定するのではなく、コードが必要なことを正確に実行するようにすべきです。設定ファイルが欲しい場合は、Claudeに追加するよう伝えることができます。

**サードパーティやオープンソースモデルを使えますか？**

はい。Agent HarborはClaude API互換のモデルエンドポイントに対応しています。`.env`ファイルで以下の環境変数を設定してください：
...
Claude Codeに聞いてください。「スケジューラーが動いていないのはなぜ？」「最近のログには何がある？」「このメッセージに返信がなかったのはなぜ？」これがAgent Harborの基盤となるAIネイティブなアプローチです。
